...
const INITIAL_TEMPLATES = [
    {
        ...
        bodyHtml: `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 650px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; background-color: #ffffff; color: #334155; line-height: 1.6;"><div id="welcome-header-container" style="text-align: center; background-color: #1b3d8b; line-height: 0;">{{header_banner}}</div><div style="padding: 32px 28px;"><h2 style="color: #0f172a; margin-top: 0; font-size: 20px; font-weight: 700; margin-bottom: 20px;">¡Hola equipo {{empresa}}! 😊</h2><p style="margin-bottom: 16px;">Mi nombre es <strong>{{mi_nombre}}</strong> y seré quién les acompañe en esta etapa de implementación de nuestro módulo <strong>Control de Asistencia</strong>.</p><p style="margin-bottom: 20px;">Como primer paso, realizaremos nuestra reunión <strong>"kick off"</strong>, la cual consiste en una instancia virtual para conocernos y revisar los puntos necesarios para comenzar el proceso de implementación.</p><div style="text-align: center; margin: 32px 0;"><a href="{{agenda_url}}" target="_blank" style="display: inline-block; background-color: #eff6ff; color: #1d4ed8; font-weight: 700; font-size: 15px; text-decoration: underline; padding: 12px 24px; border-radius: 8px; border: 1px solid #bfdbfe;">👉 Puedes hacer click aqui para agendar tu primera sesión.</a></div><p style="margin-bottom: 16px;">En caso de que no puedas asistir a la reunión programada, coméntamelo y así podremos agendar una nueva fecha.</p><p style="margin-bottom: 24px;">Quedo atenta y disponible para ayudarte.</p><div style="border-top: 1px solid #f1f5f9; padding-top: 16px; margin-top: 24px;"><p style="margin: 0 0 4px 0; color: #64748b;">Saludos,</p><p style="margin: 0; font-size: 15px; font-weight: 800; color: #1e3a8a;">Tu experiencia con Buk</p></div></div><div id="welcome-footer-container" style="text-align: center; background-color: #ffffff; line-height: 0;">{{footer_banner}}</div></div>`,
...
class FirebaseService {
    ...
    /**
     * Ejecuta una petición fetch con fallback al background script para evadir bloqueos de CSP en Gmail (Firefox)
     */
                ...
                return new Promise((resolve, reject) => {
                    chrome.runtime.sendMessage({
                        ...
                        if (chrome.runtime.lastError) {
                            return reject(new Error(chrome.runtime.lastError.message || err.message));
                        }
                        if (!res || !res.success) {
                            return reject(new Error(res?.error || err.message));
                        }
                        resolve({
                            ok: res.ok,
                            status: res.status,
    ...
    async deleteTemplate(id) {
        ...
        if (config.projectId && config.apiKey) {
            try {
                const url = `https://firestore.googleapis.com/v1/projects/${config.projectId}/databases/(default)/documents/templates/${id}?key=${config.apiKey}`;
                const response = await this._safeFetch(url, { method: 'DELETE' });
