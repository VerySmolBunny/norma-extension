...
class MondayService {
    constructor() {
        ...
        if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
            chrome.storage.local.get(['norma_drive_folders_cache', 'friday_drive_folders_cache'], (res) => {
                this._driveFoldersCache = res.norma_drive_folders_cache || res.friday_drive_folders_cache || null;
            });
        }
    }

    async resolveVariableValueAsync(varDef, client, userSenderName = '') {
        if (!this._driveFoldersCache && typeof window !== 'undefined' && window.DriveFolderContentService) {
            this._driveFoldersCache = await window.DriveFolderContentService.getFoldersCache();
        }
        ...
        if (!val && (varDef?.source === 'drive' || k === 'link_carpeta' || k === 'link_grabaciones' || k === 'carpeta_drive')) {
            if (typeof window !== 'undefined' && window.DriveFolderContentService) {
                val = await window.DriveFolderContentService.resolveClientFolderUrl(client);
            }
        }
        return val;
    }
    ...
    async _queryMonday(query, variables = {}) {
        ...
        const response = await this._safeFetch('https://api.monday.com/v2', {
            method: 'POST',
            headers: {
    ...
    resolveVariableValue(varDef, client, userSenderName = '') {
        ...
        if (varDef.source === 'drive' || k === 'link_carpeta' || k === 'link_grabaciones' || k === 'carpeta_drive' || k === 'carpeta' || k === 'grabaciones' || k === 'drive') {
            ...
            if (this._driveFoldersCache) {
                ...
                const normName = cName.toString().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, ' ').replace(/\s+/g, ' ').trim();
